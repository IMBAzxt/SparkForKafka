...
//        SparkConf sc = new SparkConf().setAppName("SparkForHDFS").setMaster("local[*]");
    ...
    public JavaStreamingContext test(String nodeList, String group, String topic) {
//        SparkConf sc = new SparkConf().setAppName("SparkForJava").setMaster("local[*]");
        SparkConf sc = new SparkConf();
        JavaStreamingContext jssc = new JavaStreamingContext(sc, Durations.seconds(20));
        jssc.sparkContext().setLogLevel("ERROR");
        ...
        TreeMap<Integer, PartitionVO> map = MyKafkaConsumer.findLeader("dn1", 9092, topic, group);
        for (Map.Entry<Integer, PartitionVO> entry : map.entrySet()) {
            acc.put(entry.getKey(), jssc.sparkContext().accumulator(0, "partition:" + entry.getKey()));
            TopicAndPartition topicAndPartition = new TopicAndPartition(topic, entry.getKey());
            long fromOffset = entry.getValue().getFromOffset();
            if (count == 0) {
                //zookeeper中没有topic的信息，则从fromOffset开始
        ...
        KafkaUtils.createDirectStream(
            ...
            public JavaRDD<MessageAndMetadata<String, String>> call(JavaRDD<MessageAndMetadata<String, String>> rdd) throws Exception {
                ...
                .foreachRDD(rdd -> {
                    ...
                    rdd
                            .mapPartitionsWithIndex(new Function2<Integer, Iterator<MessageAndMetadata<String, String>>, Iterator<String>>() {
                                @Override
                                public Iterator<String> call(Integer partition, Iterator<MessageAndMetadata<String, String>> it) throws Exception {
                                    List<String> rs = new ArrayList<>();
                                    Accumulator accumulator = acc.get(partition);
                                    while (it.hasNext()) {
                                        accumulator.add(1);
                                        rs.add(it.next().message());
                                    }
                                    return rs.iterator();
                                }
                            }, false)
                            .filter(new Function<String, Boolean>() {
                                ...
                                public Boolean call(String s) throws Exception {
                                    ...
                                    if (arr.length != 4) {
                                        return false;
                                    } else if (!MyUtils.isNumeric(arr[3])) {
                                        return false;
                                    }
                                    return true;
                                }
                            ...
                            .mapToPair(new PairFunction<String, String, Long>() {
                                public Tuple2<String, Long> call(String s) throws Exception {
                                    String[] arr = s.split(",");
                                    return new Tuple2<>(getNameByid(arr[0]), Long.parseLong(arr[3]));
                                }
                            })
...
//                            .foreach(s -> System.out.println(s));
                            .saveAsHadoopFile(
                                    "hdfs://namenode:8020/data/output/" + System.currentTimeMillis(),
                                    Text.class,
                                    Text.class,
                                    TextOutputFormat.class
                            );
                    for (Map.Entry<Integer, Accumulator> m : acc.entrySet()) {
                        System.out.println(m.getValue().name().toString() + m.getValue().value());
                    }
                });
